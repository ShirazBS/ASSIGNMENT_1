...
setwd("~/Library/CloudStorage/OneDrive-Aarhusuniversitet/AU/Assignment 1 - Semester 3")
...
```{r rename_variables}
# renaming all IDs to subject
demo_train <- rename(demo_train, subject = Child.ID)
LU_train <- rename(LU_train, subject = SUBJ)
token_train <- rename(token_train, subject = SUBJ)

# renaming all Visits to visit
demo_train <- rename(demo_train, visit = Visit)
LU_train <- rename(LU_train, visit = VISIT)
token_train <- rename(token_train, visit = VISIT)
...
```{r extract_digits}
### extracting the visit number from data sets LU_train
...
```{r remove_points}
### Write your code here
demo_train$subject <- str_replace_all(demo_train$subject, "[.]","")
LU_train$subject <- str_replace_all(LU_train$subject, "[.]","")
token_train$subject <- str_replace_all(token_train$subject, "[.]","")
```
...
# removing unnecessary columns in demo_train
demo_train <- demo_train %>%
        select(subject, visit, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

#renaming columns 
demo_train <- rename(demo_train, nonVerbalIQ = MullenRaw)
demo_train <- rename(demo_train, verbalIQ = ExpressiveLangRaw)

# removing unnecessary columns in LU_train
LU_train <- LU_train %>%
        select(subject, visit, MOT_MLU, CHI_MLU)

# removing unnecessary columns in token_train
token_train <- token_train %>%
        select(subject, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
df_LU_token <- merge(LU_train, token_train)
asd_df <- merge(df_LU_token, demo_train, by = c("subject", "visit"))
...
# Step 1: Create a new data set with only first visits
first_visit_data <- asd_df %>% 
  filter(visit == 1) %>% 
    select(subject, ADOS, nonVerbalIQ, verbalIQ, Socialization)

# Step 2: Rename the clinical variables to indicate the first visit
first_visit_data <- first_visit_data %>% 
  rename(ADOS1 = ADOS,
         nonVerbalIQ1 = nonVerbalIQ,
         verbalIQ1 = verbalIQ,
         Socialization1 = Socialization)

# Step 3: Merge the new data set with the old data
merged_data <- asd_df %>% 
  left_join(first_visit_data, by = "subject")
